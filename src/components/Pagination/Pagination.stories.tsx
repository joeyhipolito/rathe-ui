    ...
    docs: {
      description: {
        component: [
          ...
          "- The current page button carries `aria-current=\"page\"`, because the current state is otherwise only a class name and a colour.",
          "- The ellipsis is `aria-hidden` and is not a button, because it has no destination. A tab stop there only adds keystrokes between the controls that do.",
          "- A visually hidden `role=\"status\"` region announces \"Page 4 of 12\" **on change only**. It starts empty on purpose, because populating it during the first commit makes some screen readers announce the page number on load, over the top of whatever the view was already saying.",
          "",
          "Truncation is exported as `buildPageItems` and unit-tested separately. A range that flickers between five and seven controls as you page through it moves the Next button under the user's pointer, and that kind of bug is invisible in a screenshot.",
        ].join("\n"),
      },
    },
  },
...
/** At page one, Previous is disabled, not hidden, because a control that disappears moves the row. */
